

<div class="push"></div>
  </div>
  <footer class="footer">
    <div class="footer-div">
        <div class="footer-body">
            <div>
                <h3>Sigature Cards</h3>
                
            </div>
            <div>
                <h3>Pre-designed Cards</h3>
                <ul>
                    <li>Luxury Cards</li>
                    <li>Soft Pink Cards</li>
                    <li>Gold Collections</li>
                    <li>Personalized Cards</li>
                    <li>Corporate Cards</li>
                </ul>
            </div>
            <div>
                <h3>Customize Card</h3>
                <ul>
                    <li>Design a Card</li>
                    <li>Order Custom Design</li>
                </ul>
            </div>
            <div>
                <h3>Support</h3>
                <ul>
                    <li>How To Order</li>
                    <li>File Setup Instruction</li>
                    <li>Premium Sample Pack</li>
                    <li>Print Features</li>
                    <li>FAQ</li>
                    <li>Contact Us</li>
                    <li>About RockDesign</li>
                </ul>
            </div>
        <div>
        </div>
    </div>
      <div class="footer-copyright">
        <div class="copy-right">
            <div class="left-footer">

                    <ul>
                        <li>© 2024 Singature Cards NG. All Rights Reserved.</li>
                        <li><a href="#">Privacy Policy</a></li>
                        <li><a href="#">Terms and Conditions</a></li>
                    </ul>

            </div>
            <div class="right-footer">
                <ul>
                    <li>Secured Payment</li>
                    <li>Payment Platform</li>
                </ul>
            </div>
        </div>
        
    </div>
</footer>		
        
        
        <!--begin::Javascript-->
		<script>var hostUrl = "assets/";</script>
		<!--begin::Global Javascript Bundle(mandatory for all pages)-->
        <script src="https://code.jquery.com/jquery-3.7.1.min.js" integrity="sha256-/JqT3SQfawRcv/BIHPThkBvs0OEvtFFmqPF/lYI/Cxo=" crossorigin="anonymous"></script>
		<!--end::Global Javascript Bundle-->
		<!--begin::Custom Javascript(used for this page only)-->
		<script src="<?php echo URLROOT ?>/public/js/controller.js?v=<?php echo rand(10000000000,99999999999); ?>"></script>
		<!--end::Custom Javascript-->
		<!--end::Javascript-->
	</body>
	<!--end::Body-->
</html>